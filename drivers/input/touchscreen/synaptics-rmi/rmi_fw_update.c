...
static int fwu_check_version(void)
{
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
				...
				sizeof(firmware_id));
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to read firmware ID(%d)\n", dev_name( &i2c_client->dev ), retval );
		return retval;
	}
	firmware_id[3] = 0;
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
				...
				sizeof(config_id));
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to read config ID(%d)\n", dev_name( &i2c_client->dev ), retval );
		return retval;
	}
...
static int fwu_read_f01_device_status(struct f01_device_status *status)
{
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
			...
			sizeof(status->data));
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to read F01 device status\n", dev_name( &fwu->rmi4_data->i2c_client->dev ) );
		return retval;
	}
...
static int fwu_read_f34_queries(void)
{
	...
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to read bootloader ID\n", dev_name( &i2c_client->dev ) );
		return retval;
	}
	...
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to read flash properties\n", dev_name( &i2c_client->dev ) );
		return retval;
	}
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
			...
			2);
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to read block size info\n", dev_name( &i2c_client->dev ) );
		return retval;
	}
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
			...
			count);
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to read block count info\n", dev_name( &i2c_client->dev ) );
		return retval;
	}
...
static int fwu_read_interrupt_status(void)
{
	...
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to read flash status\n", dev_name( &fwu->rmi4_data->i2c_client->dev ) );
		return retval;
	}
...
static int fwu_reset_device(void)
{
	...
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
			...
			sizeof(reset));
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to reset device, address(0x%02x)\n", dev_name( &fwu->rmi4_data->i2c_client->dev ), fwu->f01_fd.cmd_base_addr );
		return retval;
	}
	...
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to reset core driver after reflash\n", dev_name( &fwu->rmi4_data->i2c_client->dev ) );
		return retval;
	}
	return 0;
}
...
static int fwu_write_f34_command(unsigned char cmd)
{
	...
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
			...
			sizeof(cmd));
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to write command 0x%02x\n", dev_name( &fwu->rmi4_data->i2c_client->dev ), cmd );
		return retval;
	}
	return 0;
}
...
static int fwu_wait_for_idle(int timeout_ms)
{
	...
	printk( "FTUCH : Device(%s), Timed out waiting for idle status\n", dev_name( &fwu->rmi4_data->i2c_client->dev ) );
...
static int fwu_scan_pdt(void)
{
	...
	if (!f01found || !f34found) {
		printk( "FTUCH : Device(%s), Failed to find both F01 and F34\n", dev_name( &fwu->rmi4_data->i2c_client->dev ) );
		return -EINVAL;
	}
		...
		unsigned char command)
{
	...
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
			...
			sizeof(block_offset));
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to write to block number registers\n", dev_name( &fwu->rmi4_data->i2c_client->dev ) );
		return retval;
	}
	...
	for (block_num = 0; block_num < block_cnt; block_num++) {
		...
		retval = fwu->fn_ptr->write(fwu->rmi4_data,
			...
			fwu->block_size);
		if (retval < 0) {
			printk( "FTUCH : Device(%s), Failed to write block data, block%d\n", dev_name( &fwu->rmi4_data->i2c_client->dev ), block_num );
			return retval;
		}

		retval = fwu_write_f34_command(command);
		if (retval < 0) {
			printk( "FTUCH : Device(%s), Failed to write command for block %d\n", dev_name( &fwu->rmi4_data->i2c_client->dev ), block_num );
			return retval;
		}

		retval = fwu_wait_for_idle(WRITE_WAIT_MS);
		if (retval < 0) {
			printk( "FTUCH : Device(%s), Failed to wait for idle status, block%d\n", dev_name( &fwu->rmi4_data->i2c_client->dev ), block_num );
			return retval;
		}

		retval = fwu_check_flash_status();
		if (retval!= 0) {
			printk( "FTUCH : Device(%s), Flash block %d status(%d)\n", dev_name( &fwu->rmi4_data->i2c_client->dev ), block_num, retval );
			return -1;
		}
...
static int fwu_write_bootloader_id(void)
{
	...
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
			...
			sizeof(fwu->bootloader_id));
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to write bootloader ID\n", dev_name( &fwu->rmi4_data->i2c_client->dev ) );
		return retval;
	}
...
static int fwu_enter_flash_prog(void)
{
	...
	if (!f01_device_status.flash_prog) {
		printk( "FTUCH : Device(%s), Program enabled bit not set\n", dev_name( &fwu->rmi4_data->i2c_client->dev ) );
		return -EINVAL;
	}
	...
	if (!f01_device_status.flash_prog) {
		printk( "FTUCH : Device(%s), Not in flash prog mode\n", dev_name( &fwu->rmi4_data->i2c_client->dev ) );
		return -EINVAL;
	}
	...
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to read F01 device control\n", dev_name( &fwu->rmi4_data->i2c_client->dev ) );
		return retval;
	}
	...
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
			...
			sizeof(f01_device_control.data));
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to write F01 device control\n", dev_name( &fwu->rmi4_data->i2c_client->dev ) );
		return retval;
	}
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->ext_data_source)
		...
		if (retval != 0) {
			printk( "FTUCH : Device(%s), Firmware image %s not available\n", dev_name( &fwu->rmi4_data->i2c_client->dev ), FW_IMAGE_NAME );
			retval = -EINVAL;
			goto exit;
		}
	...
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to do reflash\n", dev_name( &fwu->rmi4_data->i2c_client->dev ) );
	}
...
static int fwu_start_write_config(void)
{
	...
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to write config\n", dev_name( &fwu->rmi4_data->i2c_client->dev ) );
	}
...
static int fwu_do_read_config(void)
{
	...
	for (block_num = 0; block_num < block_count; block_num++) {
		...
		if (retval < 0) {
			printk( "FTUCH : Device(%s), Failed to write read config command\n", dev_name( &fwu->rmi4_data->i2c_client->dev ) );
			goto exit;
		}

		retval = fwu_wait_for_idle(WRITE_WAIT_MS);
		if (retval < 0) {
			printk( "FTUCH : Device(%s), Failed to wait for idle status\n", dev_name( &fwu->rmi4_data->i2c_client->dev ) );
			goto exit;
		}
		...
		retval = fwu->fn_ptr->read(fwu->rmi4_data,
				...
				fwu->block_size);
		if (retval < 0) {
			printk( "FTUCH : Device(%s), Failed to read block data, block%d\n", dev_name( &fwu->rmi4_data->i2c_client->dev ), block_num );
			goto exit;
		}
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (count < fwu->config_size) {
		printk( "FTUCH : Device(%s), Not enough space (%d bytes) in buffer\n", dev_name( &rmi4_data->i2c_client->dev ), count );
		return -EINVAL;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to read config\n", dev_name( &rmi4_data->i2c_client->dev ) );
		return retval;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!fwu->ext_data_source) {
		printk( "FTUCH : Device(%s), Failed to alloc mem for image data\n", dev_name( &rmi4_data->i2c_client->dev ) );
		return -ENOMEM;
	}
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!fwu) {
		printk( "FTUCH : Device(%s), Failed to alloc mem for fwu\n", dev_name( &rmi4_data->i2c_client->dev ) );
		goto exit;
	}

	fwu->fn_ptr = kzalloc(sizeof(*(fwu->fn_ptr)), GFP_KERNEL);
	if (!fwu->fn_ptr) {
		printk( "FTUCH : Device(%s), Failed to alloc mem for fn_ptr\n", dev_name( &rmi4_data->i2c_client->dev ) );
		retval = -ENOMEM;
		goto exit_free_fwu;
	}
	...
	if (retval < 0) {
		...
		printk( "FTUCH : Device(%s), Reflash for LTS not currently supported\n", dev_name( &rmi4_data->i2c_client->dev ) );
		goto exit_free_mem;
	}
	...
	retval = sysfs_create_bin_file(&rmi4_data->input_dev->dev.kobj,
			&dev_attr_data);
	if (retval < 0) {
		printk( "FTUCH : Device(%s), Failed to create sysfs bin file\n", dev_name( &rmi4_data->i2c_client->dev ) );
		goto exit_free_mem;
	}
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		...
		if (retval < 0) {
			printk( "FTUCH : Device(%s), Failed to create sysfs attributes\n", dev_name( &rmi4_data->i2c_client->dev ) );
			retval = -ENODEV;
